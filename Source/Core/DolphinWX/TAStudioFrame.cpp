...
{
	m_inputGrid->AddInputToVector(Movie::GetCurrentFrame(), Movie::GetCurrentInputCount(), PadStatus);
}
...
void TAStudioFrame::SetInput(GCPadStatus* PadStatus)
{
	...
	int inputFrame = Movie::GetCurrentInputCount();
...
void InputGrid::GoToCurrentFrame()
{
	if (m_groupByVI)
	{
		m_firstFrameInGrid = Movie::GetCurrentFrame() - m_gridNumberOfRows / 2;
		UpdateGridValues();
	}
	else
	{
		m_firstInputInGrid = Movie::GetCurrentInputCount() - m_gridNumberOfRows / 2;
		UpdateGridValues();
	}
}
...
void InputGrid::SetInputAtRow(int row, TAStudioInput tastudioInput, u64 inputCount)
{
	...
	int diff = (m_groupByVI ? tastudioInput.FrameCount - Movie::GetCurrentFrame() : inputCount - Movie::GetCurrentInputCount());
