...
{
  if(!(address > 0x80000000 && address < 0x81800000) || (address > 0x90000000 && address < 0x94000000)) {
    address += 0x80000000;
  }
  return PowerPC::Read_F32(address);
}

std::string Read_String(u32 address, int count)
{
  return PowerPC::Read_String(address, count);
}
...
{
  return PowerPC::Write_String(text, startAddress);
}
