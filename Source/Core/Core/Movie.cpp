        ...
        if (success)
        {
            ...
            while (inProgress)
            {
                ...
                while (locNext != std::string::npos)
                {
                    ...
                    {
                        float outputFloat = PowerPC::Read_F32(readAddress); //THIS NEEDS FIXED STILL -ThatsSlick
                        finalOutput = StringFromFormat(identifier.c_str(), outputFloat);
                    }
                    else if (numBytes == 4)
                    {
                        u32 output4Bytes = PowerPC::Read_U32(readAddress);
                        finalOutput = StringFromFormat(identifier.c_str(), output4Bytes);
                    }
                    else if (numBytes == 2)
                    {
                        u16 output2Bytes = PowerPC::Read_U16(readAddress);
                    ...
                    {
                        u8 output1Byte = PowerPC::Read_U8(readAddress);
